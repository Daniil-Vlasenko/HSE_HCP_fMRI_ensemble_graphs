...
def classifier_learning_(files_in1, files_in2, files_out):
    ...
    :param files_in1: Файлы вершин
    :param files_in2: Файлы
    :param files_out:
    :return:
    """
    ...
    for i in range(379):
        for j in range(i + 1, 379):
            X = pandas.DataFrame(
                {f"v{i + 1}": pandas.concat([data_vertex1.iloc[i, :], data_vertex2.iloc[i, :]], ignore_index=True),
                 f"v{j + 1}": pandas.concat([data_vertex1.iloc[j, :], data_vertex2.iloc[j, :]], ignore_index=True),
                 f"e{i + 1}_{j + 1}": pandas.concat([data_edge1.loc[(i, j), :], data_edge2.loc[(i, j), :]],
                                                    ignore_index=True)})
